...
TEST(BuildPaddingIndexMapsTest, BasicTwoSequences)
{
    ...
    Tensor<unsigned int> input_ids({num_actual_tokens}, GPU);
    Tensor<unsigned int> max_seq_len_h({1}, CPU);
    CUDA_CHECK(cudaMemcpy(max_seq_len_h.data(), &max_seq_len,
                          sizeof(unsigned int), cudaMemcpyHostToHost));

    launch_build_padding_index_maps(input_ids, max_seq_len_h, seq_lens_d,
                                    unpad_to_padded_idx_d, seq_offsets_d);
    CUDA_KERNEL_LAUNCH_CHECK();

    Tensor<unsigned int> seq_offsets_h({num_seqs}, CPU);
    Tensor<unsigned int> unpad_to_padded_idx_h({num_actual_tokens}, CPU);
    ...
    CUDA_CHECK(cudaMemcpy(unpad_to_padded_idx_h.data(), unpad_to_padded_idx_d.data(),
                          num_actual_tokens * sizeof(unsigned int), cudaMemcpyDeviceToHost));
