...
// ------------------------------------------------------------------
static void apply_rotation_llama2_decode(const std::vector<float> &head,
                                    ...
                                    std::vector<float> &expected_v)
{
    ...
    for (unsigned int seq_idx = 0; seq_idx < batch_size; ++seq_idx)
    {
        ...
        for (unsigned int h = 0; h < q_head_num; ++h)
        {
            ...
            apply_rotation_llama2_decode(head, head_dim, history_len, kv_len, rotated);
        ...
        for (unsigned int h = 0; h < kv_head_num; ++h)
        {
            ...
            apply_rotation_llama2_decode(k_head, head_dim, history_len, kv_len, rotated_k);
...
// ------------------------------------------------------------------
// fused_reshape_rope_kvcache_llama2_decode kernel tests
// ------------------------------------------------------------------
TEST(FusedReshapeRopeKVCacheLlama2DecodeTest, BasicTwoSequences)
{
    ...
    launch_fused_reshape_rope_kvcache_llama2_decode(input_d, is_done_d, kv_lens_d, q_d, k_d, v_d);
    ...
    compute_expected_decode(is_done, kv_lens, kv_cache_len, q_head_num, kv_head_num, input,
...
TEST(FusedReshapeRopeKVCacheLlama2DecodeTest, FirstDecodeStep)
{
...
TEST(FusedReshapeRopeKVCacheLlama2DecodeTest, WithDoneSequences)
{
...
TEST(FusedReshapeRopeKVCacheLlama2DecodeTest, LargeBatchLoopOverGrid)
{
