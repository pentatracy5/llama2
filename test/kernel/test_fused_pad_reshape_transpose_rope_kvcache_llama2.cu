...
#include <kernel/fused_pad_reshape_transpose_rope_kvcache_llama2.h>
...
// ------------------------------------------------------------------
// Helpers: compute expected results on the host for the LLaMA2 RoPE
// scheme (head_dim == 128). In LLaMA2 each pair is formed by the
// i-th element and the (i + head_dim/2)-th element of the head.
// ------------------------------------------------------------------
static void compute_unpad_to_pad_idx_llama2(const std::vector<unsigned int> &q_lens,
                                               ...
                                               std::vector<unsigned int> &unpad_to_pad_idx)
{
    ...
    for (unsigned int seq_id = 0; seq_id < q_lens.size(); ++seq_id)
    {
        for (unsigned int i = 0; i < q_lens[seq_id]; ++i)
        {
            ...
            unpad_to_pad_idx[unpad_idx] = seq_id * q_cache_len + i;
        }
        accumulate_len += q_lens[seq_id];
    }
}

static void apply_rotation_llama2(const std::vector<float> &head,
                                  ...
                                  std::vector<float> &out)
{
    ...
    for (unsigned int idx_in_head = 0; idx_in_head < half_dim; ++idx_in_head)
    {
        float x0 = head[idx_in_head];
        float x1 = head[idx_in_head + half_dim];
        ...
        out[idx_in_head] = cos_value * x0 - sin_value * x1;
        out[idx_in_head + half_dim] = sin_value * x0 + cos_value * x1;
    }
}

static void compute_expected_llama2(const std::vector<unsigned int> &q_lens,
                                    ...
                                    std::vector<float> &expected_v)
{
    ...
    for (unsigned int idx = 0; idx < unpad_to_pad_idx.size(); ++idx)
    {
        unsigned int padded_idx = unpad_to_pad_idx[idx];
        ...
        for (unsigned int h = 0; h < q_head_num; ++h)
        {
            ...
            apply_rotation_llama2(head, head_dim, token_idx + history_len, kv_len, rotated);
        ...
        for (unsigned int h = 0; h < kv_head_num; ++h)
        {
            ...
            apply_rotation_llama2(k_head, head_dim, token_idx + history_len, kv_len, rotated_k);
...
// ------------------------------------------------------------------
// fused_pad_reshape_transpose_rope_kvcache_llama2 kernel tests
// ------------------------------------------------------------------
TEST(FusedPadReshapeTransposeRopeKVCacheLlama2Test, BasicTwoSequences)
{
    ...
    std::vector<unsigned int> unpad_to_pad_idx;
    compute_unpad_to_pad_idx_llama2(q_lens, q_cache_len, unpad_to_pad_idx);

    Tensor<unsigned int> unpad_to_pad_idx_d({num_input_tokens}, GPU);
    ...
    launch_fused_pad_reshape_transpose_rope_kvcache_llama2(input_d, q_lens_d, kv_lens_d,
                                                           unpad_to_pad_idx_d, q_d, k_d, v_d);
    ...
    compute_expected_llama2(q_lens, kv_lens, unpad_to_pad_idx, q_cache_len, kv_cache_len,
                            q_head_num, kv_head_num, input,
...
TEST(FusedPadReshapeTransposeRopeKVCacheLlama2Test, SingleSequenceFirstForward)
{
    ...
    const std::vector<unsigned int> q_lens = {5};
    const std::vector<unsigned int> kv_lens = {5};
    const unsigned int num_input_tokens = 5;
...
TEST(FusedPadReshapeTransposeRopeKVCacheLlama2Test, IncrementalDecoding)
{
