...
template <typename T>
__global__ void fused_pad_reshape_transpose_rope_kvcache_llama2_kernel(const unsigned int num_input_tokens,
                                                                       ...
                                                                       T *output_v)
{
    ...
    while (idx < num_input_tokens)
    {
        ...
        {
            VEC4 x4 = embed[(idx * total_head_num + head_idx) * H_D + lane_id];
            if (head_idx < q_head_num) // q
            {
                head_dim_128_warp_shuffle_rope<T>(lane_group_id, lane_id_in_group, token_idx + history_len, HEAD_DIM, kv_len, x4);
                q[((seq_idx * q_head_num + head_idx) * q_cache_len + token_idx) * H_D + lane_id] = x4;
            }
            else if (head_idx >= q_head_num && head_idx < q_head_num + kv_head_num) // k
            {
                head_dim_128_warp_shuffle_rope<T>(lane_group_id, lane_id_in_group, token_idx + history_len, HEAD_DIM, kv_len, x4);
                k[((seq_idx * kv_head_num + head_idx - q_head_num) * kv_cache_len + token_idx + history_len) * H_D + lane_id] = x4;
            }
            else // v
            {
                v[((seq_idx * kv_head_num + head_idx - q_head_num - kv_head_num) * kv_cache_len + token_idx + history_len) * H_D + lane_id] = x4;
            }
                                                            ...
                                                            Tensor<T> &v)
{
    ...
    CUDA_LAUNCH(fused_pad_reshape_transpose_rope_kvcache_llama2_kernel, n_threads, threads_per_block)(num_input_tokens,
                                                                                                      q_cache_len,
                                                                                                      kv_cache_len,
                                                                                                      q_head_num,
                                                                                                      kv_head_num,
