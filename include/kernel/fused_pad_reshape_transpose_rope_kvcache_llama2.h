...
template <typename T>
void launch_fused_pad_reshape_transpose_rope_kvcache_llama2(const Tensor<T> &input,                          // (num_input_tokens, (q_head_num + 2 * kv_head_num) * head_dim)
                                                            ...
                                                            const Tensor<unsigned int> &unpad_to_padded_idx, // (num_input_tokens) compute from kernel build_padding_index_maps
