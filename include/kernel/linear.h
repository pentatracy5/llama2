...
void launch_linear(cublasHandle_t &cublas_handle,
                   ...
                   const Tensor<void *> &A_array, // pointers to each 2-D slice of the *B* tensor used by cuBLAS
                                                  // (because we swap A/B to keep row-major semantics).
                                                  // For plain 2-D GEMM (dim==2) pass an empty Tensor<void *>.
                   const Tensor<void *> &B_array, // pointers to each 2-D slice of the *A* tensor used by cuBLAS
                                                  // (because we swap A/B to keep row-major semantics).
                                                  // For plain 2-D GEMM (dim==2) pass an empty Tensor<void *>.
                   Tensor<void *> &C_array);      // pointers to each 2-D slice of C; used only for batched GEMM
                                                  // (dim>2). For plain 2-D GEMM pass an empty Tensor<void *>.
