...
//   q_decode/qkv_decode: (batch_size, q_head_num, 1, head_size)
//   k/v        : (batch_size, kv_head_num, kv_cache_len, head_size)
//   qk         : (batch_size, q_head_num, q_cache_len, kv_cache_len)
//   qk_decode  : (batch_size, q_head_num, 1, kv_cache_len)
//
                                   ...
                                   unsigned int head_size)
{
    ...
    Tensor<T> q_decode({batch_size, q_head_num, 1, head_size}, GPU);
    Tensor<T> qk_decode({batch_size, q_head_num, 1, kv_cache_len}, GPU);
    Tensor<T> qkv_decode({batch_size, q_head_num, 1, head_size}, GPU);

    const unsigned int array_len = batch_size * q_head_num;
    Tensor<void *> q_array({array_len}, GPU);
    Tensor<void *> q_decode_array({array_len}, GPU);
    Tensor<void *> k_array({array_len}, GPU);
    Tensor<void *> v_array({array_len}, GPU);
    Tensor<void *> qk_array({array_len}, GPU);
    Tensor<void *> qk_decode_array({array_len}, GPU);
    Tensor<void *> qkv_array({array_len}, GPU);
    Tensor<void *> qkv_decode_array({array_len}, GPU);

    launch_build_q_k_v_qk_qkv_array(q, q_decode, k, v, qk, qk_decode, qkv, qkv_decode,
                                    q_array, q_decode_array, k_array, v_array,
                                    qk_array, qk_decode_array, qkv_array, qkv_decode_array);
    CUDA_KERNEL_LAUNCH_CHECK();

    // Copy pointer arrays back to host for verification.
    Tensor<void *> q_array_h({array_len}, CPU);
    Tensor<void *> q_decode_array_h({array_len}, CPU);
    Tensor<void *> k_array_h({array_len}, CPU);
    Tensor<void *> v_array_h({array_len}, CPU);
    Tensor<void *> qk_array_h({array_len}, CPU);
    Tensor<void *> qk_decode_array_h({array_len}, CPU);
    Tensor<void *> qkv_array_h({array_len}, CPU);
    Tensor<void *> qkv_decode_array_h({array_len}, CPU);

    CUDA_CHECK(cudaMemcpy(q_array_h.data(), q_array.data(),
                          array_len * sizeof(void *), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(q_decode_array_h.data(), q_decode_array.data(),
                          array_len * sizeof(void *), cudaMemcpyDeviceToHost));
    ...
    CUDA_CHECK(cudaMemcpy(qk_array_h.data(), qk_array.data(),
                          array_len * sizeof(void *), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(qk_decode_array_h.data(), qk_decode_array.data(),
                          array_len * sizeof(void *), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(qkv_array_h.data(), qkv_array.data(),
                          array_len * sizeof(void *), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(qkv_decode_array_h.data(), qkv_decode_array.data(),
                          array_len * sizeof(void *), cudaMemcpyDeviceToHost));

    const unsigned int q_stride = q_cache_len * head_size;
    const unsigned int q_decode_stride = 1 * head_size;
    const unsigned int kv_stride = kv_cache_len * head_size;
    const unsigned int qk_stride = q_cache_len * kv_cache_len;
    const unsigned int qk_decode_stride = 1 * kv_cache_len;
    const unsigned int qkv_stride = q_cache_len * head_size;
    const unsigned int qkv_decode_stride = 1 * head_size;
    ...
    for (unsigned int b = 0; b < batch_size; ++b)
    {
        for (unsigned int qh = 0; qh < q_head_num; ++qh)
        {
            ...
            void *expected_q_decode = static_cast<void *>(q_decode.data() + idx * q_decode_stride);
            ...
            void *expected_qk_decode = static_cast<void *>(qk_decode.data() + idx * qk_decode_stride);
            void *expected_qkv = static_cast<void *>(qkv.data() + idx * qkv_stride);
            void *expected_qkv_decode = static_cast<void *>(qkv_decode.data() + idx * qkv_decode_stride);

            ASSERT_EQ(q_array_h.data()[idx], expected_q)
                << "q pointer mismatch at batch=" << b << ", q_head=" << qh;
            ASSERT_EQ(q_decode_array_h.data()[idx], expected_q_decode)
                << "q_decode pointer mismatch at batch=" << b << ", q_head=" << qh;
            ...
            ASSERT_EQ(qk_decode_array_h.data()[idx], expected_qk_decode)
                << "qk_decode pointer mismatch at batch=" << b << ", q_head=" << qh;
            ASSERT_EQ(qkv_array_h.data()[idx], expected_qkv)
                << "qkv pointer mismatch at batch=" << b << ", q_head=" << qh;
            ASSERT_EQ(qkv_decode_array_h.data()[idx], expected_qkv_decode)
                << "qkv_decode pointer mismatch at batch=" << b << ", q_head=" << qh;
        }
    }
}
